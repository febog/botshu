...
class PartnerPlusService {
  ...
  constructor({ chatClient, socketClient, storageService, userId }) {
    this.#chatClient = chatClient; // Calls say(), onSub(), onResub()
  ...
  updateWidget() {
    ...
    this.#socketClient.emit('points-update', `${this.getCounter()}`);
  }
  ...
  async #saveCount(count) {
    ...
    await this.#storageService.storeJsonToBlobStorage(countData, this.#countDataBlob);
  }

  async #loadCount() {
    // Read from storage
    const countData = await this.#storageService.readJsonFromBlobStorage(this.#countDataBlob);
