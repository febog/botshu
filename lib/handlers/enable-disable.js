...
const enableDisableHandlers = {
    enableBot(p) {
        if (!p.store.isBotEnabled() && p.message.startsWith("!botshu enable")) {
            p.store.enableBot();
            p.client.say(p.channel, `BotShu is now enabled mushHii`);
        }
    },
    disableBot(p) {
        if (p.store.isBotEnabled() && p.message.startsWith("!botshu disable")) {
            p.store.disableBot();
            p.client.say(p.channel, `BotShu is now disabled PETTHEMODS`);
        }
    },
    getBotStatus(p) {
        if (p.message.startsWith("!botshu status")) {
