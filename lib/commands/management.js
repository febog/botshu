    ...
    getUserCount(client, channel, user, message, store) {
        if (message.startsWith("!botshu count")) {
            ...
            if (store.userHasStrikes(firstArg)) {
                ...
                // Username not found
                client.say(channel, `@${firstArg} has no strikes`);
            }
        }
    },
    sayHelloToMods(client, channel, user, message, store) {
        if (message === "!botshu hi") {
            if (user.username === "febog") {
                client.say(channel, `Fuck you Felipe peepoGiggles`);
            } else {
                client.say(channel, `Hello ${user["display-name"]} popCat`);
            }
        }
    },
};
