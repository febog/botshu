...
async function handleMessageLanguage(p) {
    ...
    let msg = prepareMessage(p.message);
...
function isFlaggableMessage(languageResult, message) {
    ...
    if (
        ...
        ((languageResult.confidenceScore === 1 && wordCount > 1) ||
            (languageResult.confidenceScore > 0.6 && wordCount > 2))
    ) {
        return true;
    }
    ...
    if (
        ...
        ((languageResult.confidenceScore === 1 && wordCount > 2) ||
            (languageResult.confidenceScore > 0.6 && wordCount > 3))
    ) {
        return true;
    }
...
function flagUser(client, channel, user, store) {
    if (!store.userHasStrikes(user.username)) {
        ...
        client.say(
            channel,
            `Hello @${user.username} please keep the chat in English. Why? - !whyeng [WARNING]`
        );
        ...
        client.ban(channel, user.username, "[BotShu] Chat not in English");
    }
}
...
function prepareMessage(message) {
    ...
    // Remove numbers
    let anyNumbers = /\b\d+\b/;
    words = words.filter((word) => !anyNumbers.test(word));

    // Remove Mushu emotes
    let mushuEmote = /\bmushu\w+\b/;
