...
async function handleMessageLanguage(p) {
    ...
    let flaggableMessage = isFlaggableMessage(languageResult, msg);
    ...
    if (!p.store.isProduction() || flaggableMessage) {
        ...
        console.log(
            `${p.user["display-name"]}: ${msg} (${languageResult.name} (${languageResult.confidenceScore}) detected)`
        );
    }
}
 ...
 */
function flagUser(p) {
    if (!p.store.userHasStrikes(p.user.username)) {
        // No strikes, first warning
        p.store.addUserStrike(p.user.username); // First strike
        p.client.say(
            p.channel,
            `Hello @${p.user["display-name"]} please keep the chat in English. Why? - !whyeng [WARNING]`
        );
    } else if (p.store.getUserStrikes(p.user.username) === 1) {
        // Second strike, last warning
        p.store.addUserStrike(p.user.username); // Second strike
        p.client.say(
            p.channel,
            `[LAST WARNING] @${p.user["display-name"]} ENGLISH ONLY PLEASE. Why? - !whyeng`
        );
        p.client.timeout(
            p.channel,
            p.user.username,
            TIMEOUT_TIME,
            "[LAST WARNING] Chat not in English"
        );
    } else if (p.store.getUserStrikes(p.user.username) >= 2) {
        // This is the third or more time, timeout
        p.client.ban(p.channel, p.user.username, "[BotShu] Chat not in English");
    }
}
