    ...
    function getBotStatus(p) {
        if (p.message === BOTSHU_STATUS_COMMAND) {
            p.client.say(
                ...
                }. The stream is currently ${p.botshu.modules.stream.isStreamLive ? "Live" : "Offline"
                }. Running BotShu version ${process.env.npm_package_version} - (${p.store.isProduction() ? "prod" : "dev"})`
            );
        }
    },
];
