...
const partnerPlusHandlers = [
    ...
    function plusPointsGoalSet(p) {
        if (p.message.startsWith(PARTNER_PLUS_WIDGET_SET_GOAL)) {
            const points = Number(p.words[2]);
            p.store.state.plusPoints.setGoalPoints(points);
            p.client.say(p.channel, `Partner Plus Points goal set to ${points}`);
        }
    },
];
