...
module.exports = {
    ...
    addCooldownFlag(key) {
        if (this.logStore) console.log("addCooldownFlag triggered");
        if (!this.state.cooldowns.has(key)) {
            this.state.cooldowns.set(key, 1);
        }
    },
    hasCooldownFlag(key) {
        if (this.logStore) console.log("hasCooldownFlag triggered");
        return this.state.cooldowns.has(key);
    },
    clearCooldownFlag(key) {
        if (this.logStore) console.log("clearCooldownFlag triggered");
        return this.state.cooldowns.delete(key);
    },
};
