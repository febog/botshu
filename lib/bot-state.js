...
module.exports = {
    ...
    initBot() {
        // Things that happen when the bot starts to run

        // Get FFZ channel emotes
        const ffzResponse = await axios.get(
            "https://api.frankerfacez.com/v1/room/id/211256106"
        );
        let ffz = ffzResponse.data.sets["256004"].emoticons.map((e) => e.name);
        this.state.emoteList = this.state.emoteList.concat(ffz);

        // Get BTTV Global emotes, undocumented API.
        const bttvGlobalResponse = await axios.get(
            "https://api.betterttv.net/3/cached/emotes/global"
        );
        let bttvGlobal = bttvGlobalResponse.data.map((e) => e.code);
        this.state.emoteList = this.state.emoteList.concat(bttvGlobal);
    },
    isProduction() {
